...
type OpsJob struct {
	...
	Status      string    `gorm:"column:status;type:varchar(50);not null" json:"status"`           // 任务状态：pending, running, completed, failed
...
func (j *OpsJob) BeforeCreate(tx *gorm.DB) error {
	if j.ID == 0 {

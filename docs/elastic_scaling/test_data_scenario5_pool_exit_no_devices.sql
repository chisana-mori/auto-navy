...
(1, 'ESO20241201123459', '策略触发-退池-production-cluster-total-无设备告警', '策略 ''CPU Low Usage Scale In'' 触发退池操作，但无可退池设备匹配。集群：production-cluster，资源类型：total，需要退池设备：2台，实际匹配：0台。请检查集群设备状态。', 'elastic_scaling', 'pending', 'system/auto', datetime('now'), datetime('now'));
...
(1, 1, 1, 1, 'pool_exit', 'total', 0, 'CPU使用率: 28.0%', 'CPU阈值: 30.0%', datetime('now'), datetime('now'));

-- 注意：此场景不会有ng_order_device记录，因为没有匹配到设备
...
(1, 1, 1, 'total', datetime('now'), 'CPU使用率: 28.0%', 'CPU阈值: 30.0%', 'order_created_no_devices', 1, '连续3天CPU使用率低于30%，但无可退池设备匹配，生成告警订单', datetime('now'), datetime('now'));
...
-- 显示设备状态（应该显示没有running状态的设备）
SELECT
    id, ci_code, ip, status,
    CASE
        WHEN status = 'running' THEN '运行中'
        WHEN status = 'in_stock' THEN '库存'
